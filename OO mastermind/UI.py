...
class MainPage(tk.Frame):
    ...
    def __init__(self, parent, controller):
        ...
        startGameButton = tk.Button(frameOnderMainpage, text="start spel AI vs player", height=5, width=23, fg="white", bg="black",
...
class AIvsPlayer(tk.Frame):
    ...
    def __init__(AIvsPlayer, parent, controller):
        ...
        chooseAlgorithm1Button = tk.Button(onderinMiddenFrame, font='bold', text='BAD algorithm', height=2, width=16, command=lambda: algorithmConfig(1), bg='red')
        chooseAlgorithm2Button = tk.Button(onderinMiddenFrame, font='bold', text='REGULAR algorithm', height=2, width=16, command=lambda: algorithmConfig(2), bg='white')
        chooseAlgorithm3Button = tk.Button(onderinMiddenFrame, font='bold', text='GOOD algorithm', height=2, width=16, command=lambda: algorithmConfig(3), bg='white')
        ...
        autofeedbackButtonOff = tk.Button(onderinMiddenFrame, text='off', height=2, width=5, command=lambda: configAutoFeedback(False), bg='red')
        autofeedbackButtonOn = tk.Button(onderinMiddenFrame, text='on', height=2, width=5, command=lambda: configAutoFeedback(True), bg='white')

        """In frame 5 zijn de knoppen te vinden voor het bepalen van de geheime code en de enter button"""
        tekstBoven4Pinnen = tk.Label(middeninLinksFrame, text="autofeedback OFF/ON                                  Algoritme BAD/GOOD                              pin1   pin2    pin3    pin4                                                                                ", font="Arial 14", bg='grey')
        ...
        returnToHomepageButton = tk.Button(onderinMiddenFrame, text="Return to mainpage", height=5, width=23, fg="white", bg="black",
        ...
        enterButton = tk.Button(onderinMiddenFrame, text='ENTER', height=2, width=5, fg='black', command=lambda: enter(), bg='grey')
        ...
        tekstBoven4Pinnen.pack(side=tk.BOTTOM, anchor="w")
        ...
        def enter():
            if AIvsPlayer.freezeFrame == False:
                ...
                if AIvsPlayer.firstTurn:
                    ...
                    AIvsPlayer.game = Game(secretCode, AIvsPlayer.AIopponent, AIvsPlayer.opponent)  # self.code is hier de geheime code!
                ...
                if AIvsPlayer.autoFeedback:
                    ...
                    if str(AIvsPlayer.opponent) == '1':
                        guess = AIvsPlayer.game.update(inputFeedbackPins, AIvsPlayer.autoFeedback)  # self.code is hier het antwoord op de laatste gok
                        uitkomstAlgorithm.config(text="Jouw geheime code is {}, {}, {}, {} geraden in {} beurten!".format(guess[0][0], guess[0][1], guess[0][2], guess[0][3], guess[2]))
                    else:
                        while guess[3] == False:
                            guess = AIvsPlayer.game.update(inputFeedbackPins, AIvsPlayer.autoFeedback)  # self.code is hier het antwoord op de laatste gok
...
class SpelUitleg(tk.Frame):
    ...
    def __init__(self, parent, controller):
        ...
        statusbar = tk.Label(self, bd=1, relief=tk.SUNKEN, padx=10, pady=20,
                             bg="light blue", text="Copyright© Freek Gerrits Jans")
        button1 = tk.Button(frame6, text="Terug naar mainpage", height=5, width=23, fg="white", bg="black",
